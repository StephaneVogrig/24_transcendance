import { AppLayout } from './AppLayout'; // Assuming AppLayout is in the same directory or accessible.
...
// Instance de AppLayout pour gérer la mise en page globale, introduite dans router.ts
 ...
 */
function handleLinkClick(event: MouseEvent): void { // Common in both versions
 ...
 */
function addNavigationListeners(element: HTMLElement): void { // Common in both versions
  element.querySelectorAll('a[data-route]').forEach(link => { //
    link.removeEventListener('click', handleLinkClick as EventListener); // Prevent duplicate listeners
 ...
 */
export function startRouter(): void { // Common in both versions
  ...
  window.addEventListener('popstate', () => { //
    ...
    // Lors d'un popstate, l'URL est déjà celle souhaitée, on la navigue telle quelle, y compris query params and hash.
    navigate(window.location.pathname + window.location.search + window.location.hash, false); // Adapted from router_copy.ts for full URL
  });

  // Gère la navigation initiale avec l'URL complète lorsque l'application est chargée.
  navigate(window.location.pathname + window.location.search + window.location.hash); // Adapted from router_copy.ts for full URL
}